...
- Return ONLY the 3 captions, numbered. No explanation.`;
  ...
  return (
    <>
      ...
      {isModalOpen && (
        isMobile ? (
          <MobileBottomSheet isOpen={true} onClose={handleCloseModal} maxHeight="90vh">
            <div 
              ref={cardRef}
              className="relative w-full h-full flex flex-col overflow-hidden min-h-[60vh]"
              style={{
                background: 'var(--color-bg-surface)',
              }}
            >
            ...
            <div 
              className={`flex items-center justify-between px-4 py-3 relative shrink-0 ${isMobile ? 'border-t border-border-subtle sticky bottom-0 z-20 pb-safe' : 'border-t'}`}
              style={{ 
                borderColor: 'var(--color-border)',
                background: 'var(--color-bg-surface)',
            ...
            {selectedDuration && (
              <div 
                className={`px-4 py-2 text-xs italic ${isMobile ? 'border-none' : 'border-t'}`}
                style={{ 
